...
for idx in $EXIST_IDX; do
    ...
    cam_ver_year=$(sudo i2ctransfer -f -y $cam_bus w2@$cam_addr 0x30 0x00 r1 | sed 's/0x//g')
    cam_ver_month=$(sudo i2ctransfer -f -y $cam_bus w2@$cam_addr 0x30 0x01 r1 | sed 's/0x//g')
    cam_ver_rev=$(sudo i2ctransfer -f -y $cam_bus w2@$cam_addr 0x30 0x02 r1 | sed 's/0x//g')
    cam_ver_build=$(sudo i2ctransfer -f -y $cam_bus w2@$cam_addr 0x30 0x03 r1 | sed 's/0x//g')
	   ...
	   ! echo " $DES_RESET " | grep -q " $des_idx "; then
		...
		for j in $EXIST_IDX; do
			...
			if [ "$des_j" = "$des_idx" ]; then
				...
				sudo i2ctransfer -f -y "$ser_bus" w3@"$ser_addr" 0x03 0x30 0x48
				# sudo i2ctransfer -f -y "$ser_bus" w3@"$ser_addr" 0x03 0x30 0x40
			fi
		done
